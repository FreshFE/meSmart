...
use meSmart\Utils\Debug as Debug;
...
class Db {
    ...
    public static function getInstance() {
        $args = func_get_args();
        return static::get_instance_of(__CLASS__, 'factory', $args);
    }
    ...
    public static function get_instance_of($name, $method = '', $args = array())
    {
        ...
        // 获得标识
        $identify = empty($args) ? $name . $method : $name . $method . static::to_guid_string($args);
    ...
    private function parseConfig($db_config='') {
        if ( !empty($db_config) && is_string($db_config)) {
            ...
            // 如果配置为空，读取配置文件设置
            if( Config::get('DB_DSN') && 'pdo' != strtolower(Config::get('DB_TYPE')) ) { // 如果设置了DB_DSN 则优先
                $db_config =  $this->parseDSN(Config::get('DB_DSN'));
            }else{
                $db_config = array (
                    'dbms'      =>  Config::get('DB_TYPE'),
    ...
    protected function initConnect($master=true) {
        if(1 == Config::get('DB_DEPLOY_TYPE'))
            // 采用分布式数据库
    ...
    protected function parseWhereItem($key,$val) {
        ...
        if(is_array($val)) {
            ...
            //对字符串类型字段采用模糊匹配
            if(Config::get('DB_LIKE_FIELDS') && preg_match('/('.Config::get('DB_LIKE_FIELDS').')/i',$key)) {
                $val  =  '%'.$val.'%';
    ...
    public function buildSelectSql($options=array()) {
        ...
        if(Config::get('DB_SQL_BUILD_CACHE')) { // SQL创建缓存
        ...
        if(isset($key)) { // 写入SQL创建缓存
            S($key,$sql,array('expire'=>0,'length'=>Config::get('DB_SQL_BUILD_LENGTH'),'queue'=>Config::get('DB_SQL_BUILD_QUEUE')));
        }
        return $sql;
    }
