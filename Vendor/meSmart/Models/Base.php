...
class Base {
    ...
    public function flush() {
        ...
        if(Config::get('DB_FIELD_VERISON')) {
     ...
     */
     public function create($data='',$type='') {
        ...
        if(isset($fields)) {
            ...
            // 判断令牌验证字段
            if(Config::get('TOKEN_ON'))   $fields[] = Config::get('TOKEN_NAME');
            foreach ($data as $key=>$val){
        ...
        // 表单令牌验证
        if(Config::get('TOKEN_ON') && !$this->autoCheckToken($data)) {
    ...
    public function autoCheckToken($data) {
        if(Config::get('TOKEN_ON')){
            ...
            // 开启TOKEN重置
            if(Config::get('TOKEN_RESET')) unset($_SESSION[$name][$key]);
            return false;
        }
        return true;
    }
    ...
    protected function parseSql($sql,$parse) {
        ...
        if(true === $parse) {
            ...
            $sql    =   strtr($sql,array('__TABLE__'=>$this->getTableName(),'__PREFIX__'=>Config::get('DB_PREFIX')));
        }
        $this->db->setModel($this->name);
        return $sql;
    }
    ...
    public function getTableName() {
        if(empty($this->trueTableName)) {
            ...
            if(!empty($this->tableName)) {
                ...
                $tableName .= File::trans_name($this->name);
            }
